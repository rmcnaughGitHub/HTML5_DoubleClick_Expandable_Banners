...
run = {
	...
	openPanelContainer: document.getElementById('openPanelContainer'),
	...
	//ANIMATION FUNCTIONS
	aniOne: function(){//top: -490, left: -280
		TweenLite.to(img1, run.time * 2.6, {delay: 0, scale:0.66, x: -20, y: -125, transformOrigin: "50% 50%", force3D: true, rotationZ:0.01, ease:Power3.easeInOut});
		TweenLite.to(whiteBlock, run.time - 0.8, {delay: run.time * 1.2, bottom: 0, ease:Circ.easeOut});
		TweenLite.to(text1, run.time - 0.8, {delay: run.time * 1.0, top: 0, ease:Circ.easeOut});
		TweenLite.to(cta, run.time, {delay: run.time * 1.4, opacity: 1, ease:Power4.easeOut});
		TweenLite.to(run.root,0, {delay:run.time * 1.5, onComplete: function(){
			ctaOver.style.top = '526px';
		}});
		//
		//console.log('aniOne');
	},
	...
	resetAniOne: function(){
		...
		TweenLite.set(img1, {scale:1, top: -370, left: -262, x:0, y:0, transformOrigin: "50% 50%"});
		TweenLite.set(whiteBlock, {bottom: -186, });
		TweenLite.set(text1, {top: 600});
	...
	aniTwo: function(){
		run.replayTime = 1;//allow replay after first time//top: -500, left: -209
		TweenLite.to(closePanel, run.time * .5, {width: 497, height: 598, ease:Circ.easeOut, onComplete: function(){
			TweenLite.to(img1_exp, run.time * 2.6, {delay: 0, scale:.69, x: -13, y: -12, transformOrigin: "50% 50%", force3D: false, rotationZ:0.01, ease:Power3.easeInOut});
			TweenLite.to(whiteBlock_exp, run.time - 0.3, {delay: run.time * 1.2, bottom: 0, ease:Power4.easeOut});
			TweenLite.to(text1_exp, run.time - 0.4, {delay: run.time * 1.1, top: 0, ease:Power4.easeOut});
			TweenLite.to(logo_exp, run.time - 0.4, {delay: run.time * 1.1, top: 0, ease:Power4.easeOut});
			TweenLite.to(cta_exp, run.time * 1.5, {delay: run.time * 1.4, opacity: 1, ease:Power4.easeOut});
			TweenLite.to(run.root,0, {delay:run.time * 1.5, onComplete: function(){
				ctaOver_exp.style.top = '529px';
			}});
		}});
		//
		//console.log('aniTwo');
	...
	resetAniTwo: function(){
		...
		TweenLite.set(img1_exp, {scale:1, top:-473, left: -188, x:0, y:0, transformOrigin: "50% 50%"});
		TweenLite.set(whiteBlock_exp, {bottom: -230, });
		TweenLite.set(text1_exp, {top: 500});
		TweenLite.set(logo_exp, {top: 500});
		TweenLite.set(cta_exp, {opacity: 0});
		TweenLite.set(closePanel, {width: 0, height: 0});
